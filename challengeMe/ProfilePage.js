  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.statsContainer}>
                {Object.entries(user.quizStats).map(([key, { icon, score, maxScore }]) => (
                    <View style={styles.statRow} key={key}>
                        <Icon name={icon} size={20} style={styles.icon} />
                        <Text style={styles.category}>{key}</Text>
                        <View style={[styles.scoreBarContainer, { width: `${100 - (score / maxScore) * 100}%` }]}>
                            <View style={[styles.scoreBar, { width: `${(score / maxScore) * 100}%` }]} />
                        </View>
